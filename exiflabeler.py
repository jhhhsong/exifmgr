...
# csv format, with each row in following format:
# model_abbr, tzname, start time, end time (both in UTC)
# end is optional - if omitted, will assume to be start of next entry
def load_device_tzinfo(path):
    # format: { name : [ ( tz, start, end ), ... ], ... }
    ...
    with open(path, mode='r') as file:
        ...
        for name, tzname, start, end, *_ in reader:
            tz = get_timezone(tzname)
            tz_item = DeviceTzCfgEntry(tz, parse_time(start), parse_time(end))
            device_tzmap.get(name, []).append(tz_item)
    for name, tz_list in device_tzmap.items():
        tz_list.sort(key=lambda tz_item: tz_item.start.timestamp())
        for index in range(1, len(tz_list)):
            prev = tz_list[index - 1]
            cur = tz_list[index]
            if prev.end > cur.start:
                print('[Loading device tzinfo for %s - Conflict detected: configured intervals overlap for %s, %s]' %(name, prev.tz, cur.tz))
    return device_tzmap
...
# format:
# * mainname == DSC<timestamp>['<precision>][_<model_abbr>]
# * followed by modifier suffix: [_<modifier>]
...
STRUCTURED_IMGNAME_CHECK = re.compile('DSC(?P<timestamp>[0-9]{10})(_(?P<model_abbr>[A-Z][A-Z0-9]*))?(_(?P<modifier>[a-z0-9\(\)\[\],-]+))?')
#StructuredImageNameInfo = namedtuple('StructuredImageNameInfo', 'timestamp', 'timestamp_precision', 'model_abbr')
class StructuredImageNameInfo(
    ...
    def from_parse(regex):
        return StructuredImageNameInfo(
            ...
            model_abbr=regex.group('model_abbr'),
        )
